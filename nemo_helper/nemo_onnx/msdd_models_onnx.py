...
from .clustering_diarizer_onnx import ClusteringDiarizerOnnx
try:
...
class ClusterEmbeddingOnnx(torch.nn.Module):
    """
    ...
    def __init__(
        ...
        #######################
        # Update for onnx support
        self.clus_diar_model = ClusteringDiarizerOnnx(cfg=self.cfg_diar_infer, speaker_model=self._speaker_model)
...
class NeuralDiarizerOnnx(LightningModule):
    """
    ...
    def __init__(self, cfg: Union[DictConfig, NeuralDiarizerInferenceConfig]):
        ...
        self.clustering_embedding = ClusterEmbeddingOnnx(
    ...
    def run_overlap_aware_eval(
        ...
        for k, (collar, ignore_overlap) in enumerate(self.diar_eval_settings):
            ...
            self.refs.append(all_reference)
            self.hyps.append(all_hypothesis)
            output = score_labels(
                rttm_map,
                all_reference,
                all_hypothesis,
